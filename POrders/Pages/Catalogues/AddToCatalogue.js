...
function AddRows(rowObj) {
    ...
    let products = [];
    for (let i = 26; i < rowObj.length; i++) {
        let row = rowObj[i];
        let product = {};
        ...
        product['CustomGroup'] = row[config['Custom Group']];

        if (typeof (product['Class']) !== 'undefined') {
            if (product['Class'].trim() === 'solution' || product['Class'].trim() === 'printer' || product['Class'].trim() === 'accessory') {
