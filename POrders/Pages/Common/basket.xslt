     ...
     <table class="table table-striped table-bordered table-hover nowrap dataTable no-footer dtr-inline" id="dataTable" style="width: 100%"  cellspacing="0">
       ...
       <tbody>
         <xsl:for-each select="/dsQueryResponse/Rows/Row">
           ...
           <xsl:if test="@Class='solution'">
             <xsl:value-of select="ddwrt:SetVar('Qtities', ddwrt:GetVar('Qtities') + number(translate(@Qty, ',', '')))"/>
             ...
             <xsl:if test="@UpfrontCharge != ''">
               <xsl:value-of select="ddwrt:SetVar('TotalUpfrontCharge', ddwrt:GetVar('TotalUpfrontCharge') + number(translate(@UpfrontCharge, ',', '')) * number(translate(@Qty, ',', '')))"/>
             </xsl:if>
           </xsl:if>
           <xsl:if test="@Class='printer' or @Class = 'accessory'">
           </xsl:if>
           <xsl:if test="not(@Class='printer' and number(translate(@Qty, ',', '')) = 0)">
           <tr class="{@Class}">
             <!-- Index 0: Solution Number -->
             <td>
               <xsl:value-of disable-output-escaping="yes" select="@CustomGroup" />
             </td>
       ...
       <tfoot>
         <tr>
           ...
           <td style="background-color:#0096D6; display:none; color:#fff" id="poTotalBaseFee">
